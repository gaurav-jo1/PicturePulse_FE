...
const Navbar = () => {
  ...
  const { data: userinfos, isLoading, isError,} = useQuery(["userinfos"],() =>
    { return getInfo("https://picturepulse.up.railway.app/userinfo/").then((t) => t.json());}
    , { enabled: !loading }
  );
    ...
    <div className="Navbar_container">
      ...
      <div className="Navbar_container-Box3">
        <ul className={`Navbar_container-Box3-icons_${theme}`}>
          ...
          {userinfos && userinfos?.map((userinfo) => (
            <li className="Navbar_profile_logout" key={userinfo.user}>
                {userinfo.picture ? <img onClick={() => navigate("/profile")} src={`https://picturepulse.up.railway.app${userinfo.picture}`} alt={userinfo.user} width="30" height="30"/> :<img onClick={() => navigate("/profile")} src={no_profile} alt="no profile" width="30" height="30"/>}
              <ul>
        ...
        <ul className={`Navbar_container-Box3-icons_two_${theme}`}>
          {userinfos && userinfos?.map((userinfo) => (
            <li className="Navbar_profile_logout" key={userinfo.user}>
                {userinfo.picture ? <img onClick={() => navigate("/profile")} src={`https://picturepulse.up.railway.app${userinfo.picture}`} alt={userinfo.user} width="35" height="35"/> :<img src={no_profile} onClick={() => navigate("/profile")} alt="no profile" width="35" height="35"/>}
              <ul>
                <span className="triangle"></span>
                <li onClick={() => callLogout()}>Logout <FiLogOut/> </li>
              </ul>
