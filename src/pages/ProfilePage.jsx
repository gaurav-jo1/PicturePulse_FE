...
const ProfilePage = () => {
  ...
  const { data: usermedia } = useQuery( ["usermedia"],() => {
    return fetch('https://picturepulse.up.railway.app/usermedia/', {
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + String(authTokens.access),
      }
  ...
  function callMutation() {
    ...
    axios.post("https://picturepulse.up.railway.app/usermedia/", formData, {
        headers: {
          ...
          <div className="user-userinfo">
            {( userinfos?.map((userinfo) => (
                <div key={userinfo.user} className="user_profile_picture-container">
                  {userinfo.picture ? <img src={`https://picturepulse.up.railway.app${userinfo.picture}`} alt={userinfo.user} width="500" height="600" /> : <img src={no_profile} alt="noprofile" width="500" height="600" /> }
                </div>
              )))}
          </div>
        </div>
      ...
      <div className={`Profiepage_user-media_container_${theme}`}>
        {usermedia?.map((images) => (
            <div key={images.id} className="Profilepage_user-image_container">
              <img src={`https://picturepulse.up.railway.app${images.gallery}`} alt="" />
            </div>
          )).reverse()}
        {file && (
